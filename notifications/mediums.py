...
class DiscordWebhookMedium(MediumProtocol):
    ...
    async def _send_notification(self, notification: DiscordWebhookNotificationDTO) -> None:
        async with aiohttp.ClientSession() as session:
            webhook = discord.Webhook.from_url(notification.webhook, session=session)
            embed = discord.Embed(title=notification.subject, description=notification.body)
